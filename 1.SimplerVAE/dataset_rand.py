# -*- coding: utf-8 -*-
"""
Created on 2022/11/10 13:59

@author: Yang Fan

对于Dataframe设计的dataloader, 随机抽取任意一天的任意一支股票(与抽取一天的股票数据的不同之处在于Sampler不同，其他查表操作均相同)
处理的Dataframe格式如下
-------------------------------------------------------------------
...
class Dataset:
    def __init__(
        ...
        self.data = self.raw_data.query(
            f"{self.start-1}<TradingDay<{self.end+1}"
        )
        # Truncate stock data that retains start and end times to save memory and speed up processing.
        del self.raw_data
        self.data = self.data.set_index(keys=["TradingDay", "SecuCode"])
        self.label = pd.read_hdf(data_path[1], key="data")
        self.label = self.label.set_index(keys=["TradingDay", "SecuCode"])
        # To avoid the samples for labels that do not match the samples for features.
        self.data = self.data.join(self.label, how="left").sort_index(axis=0)
        self.res_label = self.data["Return"]
    ...
    def __len__(self):
        return self.data_arr.shape[0] - 1
